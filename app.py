...
def watchcontest(now):
	...
	# res += "\n" + "UTC: " + str(cur_time.isoformat()) + "\nLocal: " + convert_time(cur_time)
	return flag, res

def upcoming(site, now):
	now = now.replace(microsecond=0)
	then = now + datetime.timedelta(days = 7)
	para = {"start__gte": now.isoformat(), "start__lte": then.isoformat(), "resource__name__contains": site, "order_by": "start"}
	resp = requests.get(base_url, params = para, headers = header)
	if(resp.status_code == 200):
		return "following are the upcoming contests within a week:\n\n" + build_string(json.loads(resp.content.decode("utf-8")))
	else:
		return "Error " + str(resp.status_code)

def ongoing(site, now):
	now = now.replace(microsecond=0)
	para = {"start__lte": now.isoformat(), "end__gt": now.isoformat(), "resource__name__contains": site, "order_by": "start"}
	resp = requests.get(base_url, params = para, headers = header)
	if(resp.status_code == 200):
		return "following are the ongoing contests:\n\n" + build_string(json.loads(resp.content.decode("utf-8")))
	else:
		return "Error " + str(resp.status_code)
